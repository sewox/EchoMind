...
export const AudioMemoPlayer: React.FC<AudioMemoPlayerProps> = ({
  ...
  const generateMemoNarrative = () => {
    ...
    if (langCode === "tr") {
      parts.push(
        `İşte ${meetingTitle || "toplantınız"} için hazırlanan yapay zekâ sesli bülteni.`,
      );
      ...
      if (summary.key_decisions && summary.key_decisions.length > 0) {
        parts.push(
          `Alınan önemli kararlar şunlardır: ${summary.key_decisions.join(". ")}.`,
        );
      }
      if (summary.action_items && summary.action_items.length > 0) {
        const actions = summary.action_items
          .map((a) =>
            a.assignee ? `${a.assignee} sorumlu: ${a.task}` : a.task,
          )
          .join(". ");
      ...
      parts.push(
        `Here is the AI executive audio memo for ${meetingTitle || "your meeting"}.`,
      );
      if (summary.meeting_goal && summary.meeting_goal.trim().length > 0) {
      ...
      if (summary.key_decisions && summary.key_decisions.length > 0) {
        parts.push(
          `Key Decisions Agreed: ${summary.key_decisions.join(". ")}.`,
        );
      }
      if (summary.action_items && summary.action_items.length > 0) {
        const actions = summary.action_items
          .map((a) =>
            a.assignee ? `${a.assignee} assigned to ${a.task}` : a.task,
          )
          .join(". ");
  ...
  const handlePlayPause = () => {
    if (typeof window === "undefined" || !("speechSynthesis" in window)) {
      alert(
        "Tarayıcınız veya işletim sisteminiz ses sentezini (SpeechSynthesis) desteklemiyor.",
      );
      return;
    }
    ...
    <div className="flex items-center justify-between p-3.5 rounded-2xl bg-gradient-to-r from-indigo-950/40 via-purple-950/30 to-slate-900/60 border border-indigo-500/30 shadow-lg shadow-indigo-950/20 backdrop-blur-md">
      <div className="flex items-center gap-3">
        <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center text-white shadow-md shadow-indigo-500/20">
          <Volume2
            className={`w-4 h-4 ${isPlaying && !isPaused ? "animate-pulse text-amber-300" : ""}`}
          />
        </div>
        <div>
          <h4 className="text-xs font-bold text-white flex items-center gap-1.5">
            ...
            {t("summary.audioMemo.button") ||
              "🎙️ Sesli Bülteni Dinle (AI Podcast)"}
          </h4>
