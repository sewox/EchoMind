...
fn generate_synthetic_stress_audio(duration_secs: usize) -> (Vec<f32>, String) {
    ...
    (
        samples,
        format!(
            "Sentetik Toplantı Sesi ({} sn, Çoklu Harmonik)",
            duration_secs
        ),
    )
}
...
fn main() {
    ...
    println!(
        "   - İşletim Sistemi           : {} ({})",
        hw.os_type, hw.os_version
    );
    println!(
        "   - İşlemci (CPU)             : {} ({} Çekirdek / {} Mantıksal)",
        hw.cpu_brand, hw.physical_cores, hw.logical_cores
    );
    println!(
        "   - Toplam Sistem Belleği     : {:.1} GB RAM",
        hw.total_ram_gb
    );
    ...
    println!(
        ...
        if hw.has_ane { "EVET ✅" } else { "YOK" }
    );
    println!(
        "   - NVIDIA CUDA Desteği       : {}",
        if hw.has_cuda { "EVET ✅" } else { "YOK" }
    );
    println!(
        "   - AVX2 Desteği              : {}",
        if hw.has_avx2 { "EVET ✅" } else { "YOK" }
    );
    println!("   - Özet Başlık               : {}", hw.summary_headline);
    println!(
        "   - Önerilen Yapay Zekâ Modu  : {}",
        hw.recommended_ai_mode
    );
    ...
    let (mut test_pcm, source_desc) = if let Some(first_file) = audio_files.first() {
        ...
        match echomind_lib::importer::decode_audio_file_to_pcm16k(first_file) {
            ...
            Err(e) => {
                println!(
                    "   ⚠️ Gerçek dosya çözülemedi (Hata: {}), sentetik stres sesine geçiliyor...",
                    e
                );
    ...
    println!(
        "   - Tepe Genlik (Peak Amplitude): {:.4} -> {:.4} (Clipping Önleme Koruması)",
    ...
    for (idx, (offset_ms, chunk_samples)) in chunks.iter().take(5).enumerate() {
        println!(
            "     * Parça {}: Başlangıç {} ms | Örnek: {} ({:.2} sn)",
            idx + 1,
            offset_ms,
            chunk_samples.len(),
            chunk_samples.len() as f64 / 16000.0
        );
    }
    if chunks.len() > 5 {
    ...
    for (i, raw_text) in dirty_meeting_transcripts.iter().enumerate() {
        // DLP Maskeleme
        let redacted_result = echomind_lib::dlp::redact_sensitive_text(raw_text.to_string(), None)
        ...
        println!(
            "            DLP Sonucu : \"{}\" (Hassas Veri: {})",
            redacted_result,
            if had_sensitive {
                "BULUNDU & SANSÜRLENDİ 🛡️"
            } else {
                "TEMİZ"
            }
        );
        println!(
            "            Güvenlik   : {} | Güvenli Metin: \"{}\"",
            if was_injection_filtered {
                "INJECTION ENGELLENDİ 🚫"
            } else {
                "GÜVENLİ ✅"
            },
            sanitized
        ...
        assert!(
            wrapped_llm_prompt.contains("<raw_meeting_transcript_data>"),
            "LLM prompt data boundary izolasyonu bulunmalı"
        );
        println!(
            "   -----------------------------------------------------------------------------"
        );
    ...
    println!(
        "   - Toplam Test Edilen Tehlikeli Cümle: {}",
        dirty_meeting_transcripts.len()
    );
    println!(
        "   - Başarıyla Sansürlenen DLP Verisi : {}/{}",
        redacted_count,
        dirty_meeting_transcripts.len()
    );
    ...
    let (cleaned_text, removed_count) =
        echomind_lib::summarizer::clean_transcript_text(raw_meeting_notes, Some("tr".to_string()));
    println!(
        "   - Dolgu Kelimelerden Arındırılmış Metin ({} kelime temizlendi): \"{}\"",
        removed_count, cleaned_text
    );

    let temp_enc_file = std::env::temp_dir().join(format!(
        "echomind_stress_{}.bin",
        chrono::Utc::now().timestamp_millis()
    ));
    let enc_res = echomind_lib::encrypted_storage::write_encrypted_file(
        &temp_enc_file,
        cleaned_text.as_bytes(),
    );
    assert!(
        enc_res.is_ok(),
        "AES-256 şifreli dosya yazma başarılı olmalı"
    );
    ...
    assert_eq!(
        dec_res.unwrap(),
        cleaned_text.as_bytes(),
        "Şifre çözme orijinal veriyi birebir kurtarmalıdır"
    );
    ...
    println!(
        "  • Ses Süresi / Boyutu      : {:.1} sn ({} örnek)",
        test_pcm.len() as f64 / 16000.0,
        test_pcm.len()
    );
    println!(
        "  • Donanım Hızlandırıcı     : {} ({})",
        hw.gpu_name, hw.gpu_acceleration
    );
    println!(
        "  • VAD Parçalama            : {} segment oluşturuldu",
        chunks.len()
    );
